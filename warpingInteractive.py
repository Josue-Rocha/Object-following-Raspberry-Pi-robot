...
for y_source in range(0, rows):
    for x_source in range(0, cols):
	    ...
	    destPX = np.float32([[x_source], [y_source], [1]])
	    ...
	    if x_dest > 0 and y_dest > 0 and x_dest < cols and y_dest < rows:
		    ...
		    x_source_warped = int(sourcePX[0, 0] / sourcePX[2,0])
		    y_source_warped = int(sourcePX[1,0] / sourcePX[2, 0])

		    if 0 <= x_source_warped < cols and 0 <= y_source_warped < rows:
			    I_transformed[y_source, x_source, :] = I[y_source_warped, x_source_warped, :]
